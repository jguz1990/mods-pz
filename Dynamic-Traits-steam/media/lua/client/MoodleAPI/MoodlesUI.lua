    ...
    for _, moodleObj in pairs(self.moodles) do
        ...
        if lvl > 0 then
            ...
            if self:isMouseOverCustomMoodle(self.x + x, self.y + y) then
                local labelText = getText("Moodles_" .. moodleObj.name .. "_lvl" .. lvl);
                local descText = getText("Moodles_Desc_" .. moodleObj.name .. "_lvl" .. lvl);

                -- print("Label text " .. labelText)

                local lableWid = getTextManager():MeasureStringX(UIFont.Small, labelText)
                local descrWid = getTextManager():MeasureStringX(UIFont.Small, descText)
                local boxWidth = math.max(lableWid, descrWid)
                self:drawRect(x - (boxWidth + 12 + 6), y, boxWidth + 12, 32, 0.6, 0, 0, 0)
                self:drawText(labelText, x - 12 - lableWid, y + 2, 1, 1, 1, 1)
                self:drawText(descText, x - descrWid - 12, y + 32 - 2 - FONT_HGT_SMALL, 1, 1, 1, 1)
            end
