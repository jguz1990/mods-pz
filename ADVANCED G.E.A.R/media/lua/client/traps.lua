...
local theTraptoSet;
	...
	if(AlreadyTrapOnSquare == false) then
		...
		-- print( "THE TRAP TO SET " .. tostring(theTraptoSet))
		...
		local trapName = theTraptoSet:getType()
		local pName = player:getUsername()
		local x = math.floor(player:getX())
		local y = math.floor(player:getY())
		local z = math.floor(player:getZ())
		-- print("TRAP SET " .. tostring(trapName) .. " " .. tostring(pName) .. " at X-" .. tostring(x) .. ", Y-" .. tostring(y) .. ", Z-" .. tostring(z))		
		local trapLog = ("TRAP SET " .. tostring(trapName) .. " " .. tostring(pName) .. " at X-" .. tostring(x) .. ", Y-" .. tostring(y) .. ", Z-" .. tostring(z))
		-- PA_TrapLog(trapLog)		
		sendClientCommand(player, "PA_Trap", "TrapLog", {trapLog = trapLog})
	else
			...
			for i=0, zlist:size()-1 do
				...
				if(distance <= radius) and (instanceof(zombie,"IsoZombie") or instanceof(zombie,"IsoPlayer")) then
					...
					if(instanceof(zombie,"IsoZombie")) then
						-- -- if(ZombRand(radius)+1 > distance) then zombie:Kill(zombie) end
						if(ZombRand(radius)+1 > distance) then 
							if zombie:isCrawling() or ZombRand(2) == 0 then
								zombie:setHealth(0) 
							elseif not zombie:isCrawling() then
					...
					elseif (instanceof(zombie,"IsoPlayer")) then
						local radius2 = radius
						while radius2 > 0 and (ZombRand(radius2) > distance) do
							radius2 = ZombRand(radius2) - 1
							zombie:getBodyDamage():AddRandomDamage()
							print("Wound!")
							-- if radius2 < 0 then break end
						end
					end
...
function HandleTrap(player, trap)
	if(trap:getType() == "BearTrap") and (trap:getModData().isSet == true or trap:getWorldItem():getModData().isSet == true) then
		...
		elseif (instanceof(player,"IsoPlayer")) and not player:getVehicle() then	
			...
			if(ZombRand(2) == 0) then
				BP:setFractureTime(ZombRand(100))
	...
	elseif (trap:getType() == "LandMine") or (trap:getType() == "LandMineBig") then
		if(instanceof(player,"IsoZombie")) then
			...
			player:setHealth( player:getHealth() - ((ZombRand(25) + 40)/100) )
		elseif (instanceof(player,"IsoPlayer")) and not player:getVehicle() then	
			local BP = player:getBodyDamage():getBodyPart(BodyPartType.Foot_L)		
			if(ZombRand(2) == 0) then
				BP:setFractureTime(ZombRand(100))
				BP:generateDeepWound()
			...
			BP:AddDamage(ZombRand(25))
			local BP = player:getBodyDamage():getBodyPart(BodyPartType.Foot_R)		
			if(ZombRand(2) == 0) then
				BP:setFractureTime(ZombRand(100))
				BP:generateDeepWound()
			elseif (ZombRand(2) == 0) then
			...
			-- BP:AddDamage(ZombRand(25))
			local BP = player:getBodyDamage():getBodyPart(BodyPartType.LowerLeg_L)		
			...
			-- BP:AddDamage(ZombRand(25))
			local BP = player:getBodyDamage():getBodyPart(BodyPartType.LowerLeg_R)		
			if(ZombRand(3) == 0) then
				BP:setFractureTime(ZombRand(100))
				BP:generateDeepWound()
			elseif (ZombRand(3) == 0) then
			...
			-- BP:AddDamage(ZombRand(25))
			local BP = player:getBodyDamage():getBodyPart(BodyPartType.UpperLeg_L)		
			...
			-- BP:AddDamage(ZombRand(25))
			local BP = player:getBodyDamage():getBodyPart(BodyPartType.UpperLeg_R)		
			if(ZombRand(4) == 0) then
				BP:setFractureTime(ZombRand(100))
				BP:generateDeepWound()
			elseif (ZombRand(4) == 0) then
