...
function esSacMain.isValidContainer(selectedItem, player)
    if (selectedItem:getContainer() == nil) then return false end;
    ...
    local parentItem = esqlibItems.getContainerItem(selectedItem:getContainer(), player);
    if (not parentItem) then return true end;

    if parentItem:getContainer() and (parentItem:getContainer():getType() == "none" or
...
function esSacMain.getExtraLoots(player)
    ...
    local lootContainers = esqlibItems.getLootContainers(player);
